    ...
    public class UserLoginTask extends AsyncTask<Void, Void, Integer> {
        ...
        protected Integer doInBackground(Void... params) {
            ...
            try {
                ...
                String sql="select passwd from student where stu_name=";
                sql=sql+"\'";
                sql=sql+mID;
                sql=sql+"\'";
                String result=httpclient.connect_normal(sql);
               //String result=mPassword;
                if(result.equals("0")) return 0;//
                else {
                    String md5psaawd=MD5Util.getMD5String(mPassword);
                    JSONArray ja=new JSONArray(result);
                    //JSONObject ob=new JSONObject(json); //最外层是对象用这个
                    String passwd =ja.getJSONObject(0).getString("passwd");//数组只有一个对象
                    if(passwd.equals(md5psaawd)) return 2;
                    else return 1;
                }
        ...
        @Override
        protected void onPostExecute(final Integer item) {
            //super
            mAuthTask = null;
            //showProgress(false);
            ...
            if (item==2) {
                ...
                showProgress(false);
                mEmailView.setError("This ID is not exist");
